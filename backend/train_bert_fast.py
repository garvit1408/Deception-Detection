...
class DeceptionBERTModel(nn.Module):
    def __init__(self, bert_model_name="bert-base-uncased", metadata_dim=9):
...
def load_processed_data(data_dir="processed_data_balanced"):
    ...
    train_df = pd.read_csv(f"{data_dir}/processed_train_balanced.csv")
    ...
    for i, feature in enumerate(metadata_features):
        for data, name in [
            ...
            if not np.issubdtype(data[:, i].dtype, np.number):
                print(
                    f"Warning: {name} feature '{feature}' (column {i}) is not numeric: {data[:, i].dtype}"
                )
                print(f"Sample values: {data[:5, i]}")
                # Force convert to float
                data[:, i] = np.array(
                    [
                        float(str(x).replace("True", "1").replace("False", "0"))
                        for x in data[:, i]
                    ]
                )
...
def train_model(
    ...
    # Apply a stronger weight for deceptive class (fixed at 3.0)
    weight_for_0 = 3.0  # Fixed weight for deceptive class
    weight_for_1 = 1.0  # Weight for truthful class
...
def evaluate_model(model, test_dataset, device, batch_size=16):
    ...
    threshold = 0.3  # Lowered from 0.5 to 0.3 to reduce truthful bias
    ...
    ).to_csv("results/bert_full_predictions.csv", index=False)
...
def main():
    ...
    # Use full dataset - only balance the training data
    print("Using full dataset with balanced training...")

    # Balance the training data by oversampling the minority class (deceptive)
    ...
    # Oversample the deceptive class (minority)
    oversampled_deceptive = deceptive_df.sample(
        n_to_sample, replace=True, random_state=42
    )

    # Combine the truthful data with the oversampled deceptive data
    train_df = pd.concat([truthful_df, deceptive_df, oversampled_deceptive])

    # Shuffle the data
    train_df = train_df.sample(frac=1, random_state=42).reset_index(drop=True)

    print(f"Full balanced train data shape: {train_df.shape}")
    print(f"Full validation data shape: {val_df.shape}")
    print(f"Full test data shape: {test_df.shape}")

    # Check class balance in training data
    train_truthful = train_df["is_truthful"].sum()
    train_deceptive = len(train_df) - train_truthful
    print(
        f"Balanced training data: {train_truthful} truthful, {train_deceptive} deceptive"
    )
    print(f"Truthful percentage: {train_truthful / len(train_df) * 100:.2f}%")
    ...
    print("Training BERT model on full dataset...")
    model = train_model(
        model, train_dataset, val_dataset, device, batch_size, num_epochs=3
    )
